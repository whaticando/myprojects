...
class DetailView(generic.DetailView):
    ...
    # 获取数据库中的文章列表
    model = Article
    # template_name 属性用于指定哪个模板进行渲染
    template_name = 'article.html'
    # context_object_name属于用于给上下文变量取名
...
def backend(request, username):
    article_list = Article.objects.filter(author__username=username)
    return render(request, "backend/backend_index.html", {"username": username, "article_list": article_list})
...
def backend_add_article(request,username):
    ...
    if request.method == "POST":
        if request.method == "POST":
            ...
            loves = request.POST.get('loves')
            slug = request.POST.get('slug')
            category_id = request.POST.get('category')
            tags_id = request.POST.getlist('tags')
            # keywords_id = request.POST.getlist('keywords')
            print(author, title, summary, body, img_link, loves, slug, category_id, tags_id)
            extra = {}
            if category_id:
                extra["category_id"] = category_id
            # elif tags_id:
            #     extra['tags_id'] = tags_id
            # elif keywords_id:
            #      extra['keywords_id'] = keywords_id
            Article.objects.create(author=user, title=title,summary=summary, body=body, img_link=img_link, loves=loves, slug=slug,
                                 **extra).tags.set(tags_id)


            return redirect("http://127.0.0.1:8000/blog/backend")

    return render(request, "backend/backend_add_article.html", locals())
...
def edit_article(request, article_id):
    if request.method == "POST":
        ...
        if category_id:
            extra["category_id"] = category_id
        Article.objects.filter(pk=article_id).update(title=title,summary=summary, body=body, img_link=img_link, loves=loves,
                                                     slug=slug)
        Article.objects.get(pk=article_id).tags.set(tags_id)
        return redirect('http://127.0.0.1:8000/blog/backend/')


    article = get_object_or_404(Article, pk=article_id)
    article_tags = Article.objects.get(id=article_id)
    article_keywords = Article.objects.get(id=article_id)
