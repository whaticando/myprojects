...
from user.templatetags.oauth_tags import *
...
def get_comment_count(category, entry=0):
    """获取一个文章的评论总数"""
    if category == 'about':
        lis = AboutComment.objects.all()  # 关于自己页面的评论消息
    elif category == 'message':
        lis = MessageComment.objects.all()  # 我的留言页面评论信息
    else:
        lis = ArticleComment.objects.filter(belong_id=entry)  # 文章评论区，继承评论信息表
    return lis.count()
...
def find_father(dic, comment_obj):
    for k, v_dic in dic.items():  # 对字典中的每一组元素进行循环操作
        if k == comment_obj.parent:  # 如果k等于comment_obj的父节点，那么表示找到了父亲。
            dic[k][comment_obj] = {}  # 找到了父亲，认祖归宗，把自己归位到父亲下面，并给将来的儿子留个位置 # 找到了父亲，处理完毕，返回
        else:
            find_father(dic[k], comment_obj)  # 刚才没找到，剥一层，接着往下找。
...
def generate_comment_html(sub_comment_dic,category, path, s=2):
    ...
    for k, v_dic in sub_comment_dic.items():
        ...
        <div class="c-avatar"><img alt='' src="/media/avatar/default.png" class='avatar avatar-54 photo' height='54' width='54' />
        ...
        <a rel='nofollow' class='comment-reply-link' href='{5}?replytocom={1}#respond' onclick='return addComment.moveForm( "div-comment-{1}", "{1}", "respond", "{5}" )' aria-label='回复给{3}'>回复</a>
        </div></div></div>'''.format(s, k.id, k.content, k.author.nickname, k.create_date.strftime('%Y-%m-%d %H:%M:%S'),
                                     category, path)
...
def build_comment_tree(category, path, entry=0):
    ...
    for k, v in comment_dic.items():
        ...
        <div class="c-avatar"><img alt='' src="/media/avatar/32dff2a31c41fcd5f5232f2f98dec9b2.jpeg" class='avatar avatar-54 photo' height='54' width='54' />
        <div class="c-main" id="div-comment-{0}">{1}
        <div class="c-meta"><span class="c-author">{2}</span>{3}
        <a rel='nofollow' class='comment-reply-link' href='{5}?replytocom={0}#respond' onclick='return addComment.moveForm( "div-comment-{0}", "{0}", "respond", "{5}" )' aria-label='回复给{4}'>回复</a>
        </div></div></div>'''.format(
