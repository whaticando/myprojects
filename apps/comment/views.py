...
def confirm(new_content, comment_post_ID, auser):
    if comment_post_ID == 'about':
        ...
        res = ArticleComment.objects.filter(content=new_content, author=auser)
...
def AddcommentView(request):
    ...
    if request.is_ajax():
        ...
        return HttpResponse(
            '''<li class="" id="comment-"><div class="c-avatar"><img alt='' src="/media/{0} " class='avatar avatar-54 photo avatar-default' height='54' width='54' /><div class="c-main" id="div-comment-">{1}<div class="c-meta"><span class="c-author">{2}</span></div></div></div>'''.format(
                request.user.avatar,new_content, author), content_type='text/html;charset="utf-8"')
